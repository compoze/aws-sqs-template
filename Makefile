.SHELL := /usr/bin/bash
.PHONY: check-workspace check-run-deps check-install-deps install prod stage qa dev testing plan apply destroy
...
check-workspace: check-run-deps
	@if [ ! -f .terraform/terraform.tfstate ]; then \
	  ...
	  echo "$(YELLOW)Run $(BOLD)make <workspace> $(RESET)$(YELLOW)Where <workspace> is qa, stage or prod.$(RESET)"; \
	  exit 1; \
	fi
